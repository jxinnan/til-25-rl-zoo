...
class RLManager:
    ...
    def evaluate_position(self, position: Tuple[int, int], current_pos: Tuple[int, int]) -> float:
        ...
        openness_score = self.calculate_room_openness(position) * 3.0
        
        # Bonus for unvisited positions
        exploration_bonus = 2.0 if position not in self.visited_positions else 0.0
        
        # Strategic position bonus
        strategic_bonus = 1.5 if position in self.strategic_positions else 0.0
        ...
        origin_bonus = max(0, 2.0 - origin_distance * 0.1)
        ...
        movement_bonus = min(current_distance * 0.5, 1.0)
        
        return openness_score + exploration_bonus + strategic_bonus + origin_bonus + movement_bonus
