...
class RLManager:
    ...
    def __init__(self):
        ...
        # configurations.
        self.model = DQN.load("guards/banqiao_4M/banqiao_001_basic_4000000_steps")
        self.size = 16

        # observation space
        self.VIEW_WINDOW_SIZE = 1 + 7 * 2
        self.BITS_PER_TILE = 9 # walls (4), rewards (2), seen (1), within bounds (1), guards (1)
    ...
    def rl(self, observation: dict[str, int | list[int]]) -> int:
        ...
        new_scout_space = np.rot90(self.obs_scout_space, k=3-curr_direction)
        ...
        for dst_y in range(self.VIEW_WINDOW_SIZE):
            ...
            for dst_x in range(self.VIEW_WINDOW_SIZE):
                ...
                # reward
                if new_rewards_space[ori_x, ori_y] == 170 or new_rewards_space[ori_x, ori_y] == 85:
                    output_obs[output_obs_idx] = 1
                output_obs_idx += 1

                # scout
                output_obs[output_obs_idx] = self.GUARD_DISCRETE_LEVELS[new_scout_space[ori_x, ori_y]]
                output_obs_idx += 1

                # seen
                if new_rewards_space[ori_x, ori_y] != 255: # not not seen
                    output_obs[output_obs_idx] = 1
                output_obs_idx += 1
