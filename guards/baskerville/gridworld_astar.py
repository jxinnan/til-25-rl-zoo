...
def get_valid_neighbors(grid: np.ndarray, position: Tuple[int, int]) -> List[Tuple[int, int]]:
    ...
    fourwalls = grid[x,y, :4]
    directions = [(0, -1), (0, 1), (-1, 0), (1, 0)]
    guard_presence = [grid[x+i, y+j, 4] if 0 <= (x%16)+i < 16 and 0 <= y+j < 16 else 0 for i, j in directions]
    ...
    if x < 16: # vertical
        ...
        if fourwalls[0] == 0 and guard_presence[0] == 0: possible_moves.append((x,y-1)) # up
        if fourwalls[2] == 0 and guard_presence[2] == 0: possible_moves.append((x,y+1)) # down
    else: # horizontal
        possible_moves.append((x-16,y)) # switch dims
        if fourwalls[1] == 0 and guard_presence[1] == 0: possible_moves.append((x-1,y)) # left
        if fourwalls[3] == 0 and guard_presence[3] == 0: possible_moves.append((x+1,y)) # right
