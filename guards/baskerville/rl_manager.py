...
class RLManager:
    ...
    def __init__(self):
        ...
        self.obs_guards = np.zeros((self.size, self.size), dtype=np.uint8)
    ...
    def rl(self, observation: dict[str, int | list[int]]) -> int:
        ...
        elif self.last_scout_turn < 10:
            if self.temp_dest:
                scout_loc = self.temp_dest
            else:
                diff_x = int(self.last_scout[0]) - int(self.chase_pos[0])
                diff_y = int(self.last_scout[1]) - int(self.chase_pos[1])

                target_x = self.last_scout[0] + diff_x
                target_y = self.last_scout[1] + diff_y

                if target_x < 0 or target_x > self.size-1:
                    target_x = self.size - target_x%self.size
                
                if target_y < 0 or target_y > self.size-1:
                    target_y = self.size - target_y%self.size

                self.temp_dest = (target_x, target_y)
        else:
            self.temp_dest = None
            self.chase_pos = None


        # Reset Guard Presence
        self.obs_guards = np.zeros((self.size, self.size), dtype=np.uint8)
        ...
        for i in range(new_gridview.shape[0]):
            ...
            for j in range(new_gridview.shape[1]):
                ...
                if (
                    unpacked[4] == 1 and 
                    not (curr_location[0] == new_abs_x and curr_location[1] == new_abs_y) and
                    (abs(new_abs_x - curr_location[0]) + abs(new_abs_y - curr_location[1]) < 3) 
                ):
                    self.obs_guards[new_abs_x][new_abs_y] = 1

                # update visible scouts
                ...
                if tile_scout_info == 1:
                    ...
                    self.chase_pos = (curr_location[0], curr_location[1])
        ...
        astar_half_grid = np.dstack([self.obs_wall_top_space, self.obs_wall_left_space, self.obs_wall_bottom_space, self.obs_wall_right_space, self.obs_guards])
