...
class AStar:
    """A* pathfinding with rotation costs"""
    
    def __init__(self, walls: np.ndarray, size: int = 16, guard_positions: List[Tuple[int, int]] = None, avoidance_radius: int = 2):
        self.walls = walls
        self.size = size
    ...
    def heuristic(self, pos: Tuple[int, int], goal: Tuple[int, int]) -> float:
        ...
        # Add penalty for being near guards
        guard_penalty = 0
        if self.is_near_guard(pos):
            guard_penalty = 100  # High penalty for being near guards
    ...
    def get_neighbors(self, state: Tuple[int, int, int]) -> List[Tuple[Tuple[int, int, int], int]]:
        ...
        if not self.walls[direction][x][y]:
            ...
            if 0 <= new_x < self.size and 0 <= new_y < self.size:
                # Add extra cost if moving near a guard
                move_cost = 1
                if self.is_near_guard((new_x, new_y)):
                    move_cost += 50  # Discourage moving near guards
    ...
    def find_safe_position(self, start: Tuple[int, int, int], min_distance: int = 3) -> Optional[Tuple[int, int]]:
        """Find a safe position away from all guards"""
        start_x, start_y, start_dir = start
        safe_positions = []
        
        # Find all positions that are far enough from guards
        for x in range(self.size):
            for y in range(self.size):
                # Check if position is safe (not near any guard)
                is_safe = True
                for guard_pos in self.guard_positions:
                ...
                if is_safe:
                    ...
                    safe_positions.append((start_distance, (x, y)))
...
class GuardAvoidanceManager:
    """Manages guard detection and avoidance behavior"""
    ...
    def __init__(self, rl_manager):
        ...
        self.min_safe_distance = 3  # Minimum distance to maintain from guards
    ...
    def plan_avoidance(self, current_pos: Tuple[int, int], current_dir: int) -> bool:
        """Plan an avoidance route away from detected guards"""
        # Create A* with current guard positions
        astar = AStar(
...
class WaypointPlanner:
    """Plans optimal waypoints based on current map state"""
    ...
    def calculate_waypoint_value(self, waypoint: Tuple[int, int], current_pos: Tuple[int, int]) -> float:
        ...
        for guard_pos in self.rl_manager.curr_guard_pos:
            ...
            if guard_distance <= 3:  # Avoid waypoints near guards
                guard_penalty += 50 * (4 - guard_distance)  # Stronger penalty for closer guards
        
        # Role-specific bonuses
        if self.rl_manager.is_scout:
            # Scout-specific logic
            guard_bonus = self.rl_manager.guards_tracker.get(waypoint, 0)
            return base_value + guard_bonus + visit_penalty - distance_penalty - guard_penalty
        else:
            ...
            # Preferential target bonus for early game
            target_bonus = 0
            ...
            return base_value + scout_bonus + target_bonus + visit_penalty - distance_penalty - guard_penalty
    ...
    def get_visible_waypoints(self, current_pos: Tuple[int, int]) -> List[Tuple[int, int]]:
        ...
        for i in range(self.rl_manager.size):
            for j in range(self.rl_manager.size):
                # Only consider explored tiles (not hidden)
                if self.rl_manager.value[i][j] != self.rl_manager.rewards['hidden']:
                    ...
                    if (i, j) != current_pos:
                        ...
                        if dist >= 1:  # At least 1 step away
    ...
    def find_best_waypoint(self, current_pos: Tuple[int, int], current_dir: int) -> Optional[Tuple[int, int]]:
        ...
        waypoint_scores = []
        astar = AStar(self.rl_manager.obs_wall, self.rl_manager.size)
...
class NavigationManager:
    """Manages waypoint-based navigation with guard avoidance"""
    ...
    def plan_navigation(self, current_pos: Tuple[int, int], current_dir: int) -> Optional[int]:
        """Plan navigation with guard avoidance priority"""
        ...
        if self.guard_avoidance.should_activate_avoidance(current_pos):
            ...
            self.guard_avoidance.clear_avoidance()  # Clear any old avoidance
        ...
        if self.needs_new_waypoint(current_pos):
            ...
            if self.current_waypoint is None:
                return None  # No valid waypoint found
            
            # Plan path to waypoint using A*
        ...
        return None  # Path completed or no path available


# Updated RLManager class with integrated guard avoidance
class RLManager:
    SCOUT_REWARDS = {
        'empty': 0,
        'hidden': 1.2,
    ...
    def __init__(self):
        ...
        # Navigation system with guard avoidance
    ...
    def rl(self, observation_json: dict) -> int:
        ...
        # Use enhanced navigation with guard avoidance
