...
class AStar:
    def __init__(self, walls: np.ndarray, size: int = 16,
                 guard_positions: List[Tuple[int,int]] = None,
                 avoidance_radius: int = 2,
                 guard_memory: GuardMemory = None,
                 current_step: int = 0,
        ...
        # NEW: cells we want to heavily avoid this tick
        self.dynamic_obstacles = dynamic_obstacles or set()
        self.DELTA = [(1,0),(0,1),(-1,0),(0,-1)]

    def is_dead_end(self, pos: Tuple[int,int]) -> bool:
        """
        A cell is a dead-end if it has ≤1 valid neighbor (i.e. only one way in/out).
        We treat walls, current guards, and dynamic obstacles as blocking.
        """
        exits = 0
        x, y = pos
        for dx, dy in self.DELTA:
            ...
            # find which direction that wall would be in from (x,y)
            dir_idx = self.DELTA.index((dx,dy))
            if self.walls[dir_idx][x][y]:
                continue
            # guard standing there?
            if (nx, ny) in self.guard_positions:
                continue
            # dynamic obstacle there?
            if hasattr(self, 'dynamic_obstacles') and (nx,ny) in self.dynamic_obstacles:
                continue
            exits += 1
            if exits > 1:
                return False
    ...
    def get_location_penalty(self, pos: Tuple[int,int]) -> float:
        """Combine static, historical, dynamic guard penalties—and now dead-end penalty."""
        penalty = 0.0

        # static “too near” penalty
        ...
        # dynamic guard path penalty
        if hasattr(self, 'dynamic_obstacles') and pos in self.dynamic_obstacles:
            penalty += 1e3

        # **new**: heavy penalty for stepping into a dead-end
    ...
    def find_safe_position(self,
        ...
        # If there are no guards, don’t attempt a ‘safe’ reposition
        if not self.guard_positions:
            return None

        start_x, start_y, start_dir = start
        safe_positions: List[Tuple[float, Tuple[int, int]]] = []

        # If multiple guards are clustered, bump up the minimum safe distance
        if len(self.guard_positions) >= 2:
            min_guard_distance = float('inf')
            for i, g1 in enumerate(self.guard_positions):
                for g2 in self.guard_positions[i+1:]:
                    d = abs(g1[0] - g2[0]) + abs(g1[1] - g2[1])
                    min_guard_distance = min(min_guard_distance, d)
            if min_guard_distance <= 2:
                min_distance = max(min_distance, 5)
                print(f"Guards are close together (distance: {min_guard_distance}), "
                    f"increasing safe distance to {min_distance}")

        # Scan every cell
        for x in range(self.size):
            for y in range(self.size):
                # Must be at least min_distance from *each* guard
                too_close = any(
                    abs(x - gx) + abs(y - gy) < min_distance
                    for gx, gy in self.guard_positions
                )
                if too_close:
                    continue

                # Historical danger requires extra buffer
                if self.guard_memory and self.guard_memory.is_location_dangerous((x, y), self.current_step):
                    if any(
                        abs(x - gx) + abs(y - gy) < (min_distance + 2)
                        for gx, gy in self.guard_positions
                    ):
                        continue

                # Compute score: farther from start is worse, penalize historical danger,
                # *and* reward being further from guards
                start_distance = abs(x - start_x) + abs(y - start_y)
                danger_penalty = (
                    self.guard_memory.get_danger_penalty((x, y), self.current_step)
                    if self.guard_memory else 0.0
                )
                # Only apply this bonus if guards exist
                guard_distance_bonus = min(
                    abs(x - gx) + abs(y - gy)
                    for gx, gy in self.guard_positions
                ) * 0.1
        ...
        # Pick the lowest‐score (best) spot
        safe_positions.sort(key=lambda item: item[0])
        selected = safe_positions[0]
        print(f"Selected safe position: {selected[1]} (score: {selected[0]:.2f})")
        return selected[1]
...
class GuardAvoidanceManager:
    ...
    def compute_dynamic_obstacles(self,
                                  current_pos: Tuple[int,int],
                                  lookahead: int = 5) -> None:
        """
        For each guard, run a quick A* to your current_pos and
        collect the first `lookahead` cells into self.dynamic_obstacles.
        """
        dyn = set()
        for g in self.detect_guards():
            # simple A* from guard → scout, *ignoring* dynamic penalties
            guard_astar = AStar(
            ...
            path = guard_astar.find_path((g[0], g[1], 0), current_pos)
            if not path:
                continue

            # simulate the first `lookahead` moves
            x, y, d = g[0], g[1], 0
            for action in path[:lookahead]:
                if action == 0:        # forward
                    dx, dy = guard_astar.DELTA[d]
                    x, y = x+dx, y+dy
                elif action == 1:      # backward
                    bd = (d+2)%4
                    dx, dy = guard_astar.DELTA[bd]
                    x, y = x+dx, y+dy
    ...
    def get_avoidance_action(self,
        ...
        """
        Replan every tick against both static & dynamic obstacles,
        then take only the first step of that fresh path.
        """
        # 1) Update history & memory
        self.update_position_history(current_pos)
        self.update_guard_memory(self.rl_manager.step)

        # 2) Exit if no longer need avoidance
        if not self.should_avoid_guards(current_pos):
            self.reset_avoidance_state()
            return None

        # 3) Predict guards’ next few cells
        self.compute_dynamic_obstacles(current_pos, lookahead=5)

        # 4) Build a brand-new A* with dynamic_obstacles baked in
        planner = AStar(
            walls=self.rl_manager.obs_wall,
            size=self.rl_manager.size,
        ...
        path = planner.find_path((current_pos[0], current_pos[1], current_dir),
                                 # use exit_radius as your “safe” goal
                                 goal=(current_pos[0], current_pos[1]))  # hack: find nearest safe
        # (you can replace goal with a pre-computed safe_target if you like)

        if not path:
            # fallback emergency if A* fails
            return self.get_emergency_action(current_pos, current_dir)

        # Only commit to the very next action
        next_action = path[0]
        return next_action
    ...
    '''def get_avoidance_action(self,
        ...
        """
        Get the next action for guard avoidance, re‐planning if a guard moves into our next step.
        Returns None if no avoidance is needed (so caller can fall back to normal navigation).
        """
        # 1) Track history
        self.update_position_history(current_pos)

        # 2) Decide if we should still be in avoidance mode
        should_avoid = self.should_avoid_guards(current_pos)
        if not should_avoid:
            # fully exit avoidance state
            self.reset_avoidance_state()
            return None

        # 3) Check if we need to replan: either no path, exhausted path, or guard moved into next cell
        need_replan = False

        if self.avoidance_path and self.avoidance_path_index < len(self.avoidance_path):
            next_action = self.avoidance_path[self.avoidance_path_index]
            # simulate next position
            if next_action in (2, 3):  # turn left/right
                next_pos = current_pos
            else:
                move_dir = (current_dir if next_action == 0 else (current_dir + 2) % 4)
                dx, dy = self.rl_manager.DELTA[move_dir]
                next_pos = (current_pos[0] + dx, current_pos[1] + dy)

            if next_pos in self.detect_guards():
                need_replan = True
        else:
            # no existing plan or plan exhausted
            need_replan = True
        ...
        # 6) If plan complete and we’re safe, record success
