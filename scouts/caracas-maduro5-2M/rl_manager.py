...
class RLManager:
    ...
    def __init__(self):
        ...
        # configurations.
        self.model = DQN.load("scouts/caracas-maduro5-2M/caracas-maduro5_guard_2000000_steps")
        self.size = 16
        ...
        self.obs_guard_space = np.zeros((self.size, self.size), dtype=np.float32)
    ...
    def rl(self, observation: dict[str, int | list[int]]) -> int:
        ...
        Returns:
            ...
            the options.
        """
        self.obs_guard_space[self.obs_guard_space > 0] -= 0.125
        ...
        for i in range(new_gridview.shape[0]):
            ...
            for j in range(new_gridview.shape[1]):
                ...
                if tile_guard_info == 1:
                    self.obs_guard_space[new_abs_x, new_abs_y] = np.float32(1)
                else:
                    self.obs_guard_space[new_abs_x, new_abs_y] = np.float32(0)
        ...
        for dst_y in range(self.VIEW_WINDOW_SIZE):
            ...
            for dst_x in range(self.VIEW_WINDOW_SIZE):
                ...
                # guards
                output_obs[output_obs_idx] = new_guard_space[ori_x, ori_y]
                output_obs_idx += 1
