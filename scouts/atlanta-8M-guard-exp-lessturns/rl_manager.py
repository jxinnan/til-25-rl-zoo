...
class RLManager:
    ...
    def __init__(self):
        ...
        # configurations.
        self.model = DQN.load("scouts/atlanta-8M-guard-exp-lessturns/atlanta-8M-guard-exp_guard1")
        self.size = 16
    ...
    def rl(self, observation: dict[str, int | list[int]]) -> int:
        ...
        for dst_y in range(self.VIEW_WINDOW_SIZE):
            ...
            for dst_x in range(self.VIEW_WINDOW_SIZE):
                ...
                # guards
                if new_guard_space[ori_x, ori_y] > 128:
                    output_obs[output_obs_idx] = 0.5 + (new_guard_space[ori_x, ori_y] - 127)/(128*2)
                elif new_guard_space[ori_x, ori_y] < 128:
                    output_obs[output_obs_idx] = (new_guard_space[ori_x, ori_y])/(128*2)
                else: # not seen in past 16 turns
                    output_obs[output_obs_idx] = 0.5
                output_obs_idx += 1
        
        # last 3 actions were turning
        # CHANGED: to 2
