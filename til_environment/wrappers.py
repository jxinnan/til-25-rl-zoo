...
class GuardEvalWrapper(BaseWrapper[AgentID, ObsType, ActionType]):
    def __init__(self, env, running_scout, guard_classes = None, scout_class = None, chosen_astar = (-1,-1), side_astar = (-1,-1)):
        """
        Parameters
        ----------
        ...
        self.obs_scout_space = np.full((self.size, self.size), 16, dtype=np.uint8)
        self.obs_guard_space = np.full((self.size, self.size), 16, dtype=np.uint8)
        ...
        # dirty fix
        self.the_chosen_guard = None
    
    def step(self, the_chosen_action):
        if the_chosen_action is not None:
            # START SAC
            # the_chosen_action = floor(the_chosen_action)
            # END SAC
            super().step(the_chosen_action)
        ...
        for agent in self.agent_iter():
            ...
            if agent == self.the_chosen_one:
                ...
                return (
                    self.shape_obs(self.observe(self.the_chosen_one)), 
                    self._cumulative_rewards[self.the_chosen_one],
                    termination,
                    truncation,
                    self.infos[self.the_chosen_one],
        ...
        # self.obs_guard_space[self.obs_guard_space > 128] -= np.minimum(self.obs_guard_space[self.obs_guard_space > 128] - 128, 8)
        # self.obs_guard_space[self.obs_guard_space < 128] += np.minimum(128 - self.obs_guard_space[self.obs_guard_space < 128], 8)
        self.obs_scout_space[self.obs_scout_space > 16] -= 1
        self.obs_scout_space[self.obs_scout_space < 16] += 1
        self.obs_guard_space[self.obs_guard_space > 16] -= 1
        self.obs_guard_space[self.obs_guard_space < 16] += 1
        ...
        # custom_dynamic_DIST_TO_SCOUT
        rew_ego_loc = list(curr_location)
        if curr_direction == Direction.LEFT or curr_direction == Direction.RIGHT: rew_ego_loc[0] += 16
        rew_ego_loc = tuple(rew_ego_loc)
        rew_scout_loc = tuple(self.agent_locations[self.scout])
        rew_astar_half_grid = self.true_wall_grid
        rew_astar_grid = np.tile(rew_astar_half_grid, (2,1,1))
        curr_dist_to_scout = len(astar.find_path(rew_astar_grid, rew_ego_loc, rew_scout_loc))

        reward += curr_dist_to_scout * self.rewards_dict.get(
            "custom_DIST_TO_SCOUT", 0
        )
        ...
        for i in range(new_gridview.shape[0]):
            ...
            for j in range(new_gridview.shape[1]):
                ...
                if tile_scout_info == 1:
                    ...
                    if abs(i - rel_curr_location[0]) + abs(j - rel_curr_location[1]) <= 1:
                        reward += self.rewards_dict.get(
                            "custom_SCOUT_ONE_TILE", 0
                        )
        # END custom rewards from observation
        self.shape_obs(observation)
        return observation, reward, termination, truncation, info
    
    def reset(self, seed=None, options=None, chosen_guard_index=0):
        """Gymnasium-like reset function which assigns obs/action spaces to be the same for each agent.
        ...
        if self.running_scout:
            ...
            the_chosen_index = chosen_guard_index # make it deterministic
            for candidate_agent in self.agents:
                if candidate_agent == self.scout: continue
                if the_chosen_index == 0:
                    self.the_chosen_one = candidate_agent
    ...
    def shape_obs(self, obs):
        ...
        for i in range(new_gridview.shape[0]):
            ...
            for j in range(new_gridview.shape[1]):
                ...
                tile_scout_info = unpacked[5]
                if tile_scout_info == 1:
                    self.obs_scout_space[new_abs_x, new_abs_y] = np.uint8(32)
                else:
                    self.obs_scout_space[new_abs_x, new_abs_y] = np.uint8(0)
