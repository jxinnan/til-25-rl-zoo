...
class MCTSGoalPlanner:
    ...
    def __init__(self, grid_size: int = 16, exploration_constant: float = 1.414):
        self.grid_size = grid_size
...
class RLManager:
    ...
    def should_replan_goal(self, current_pos: Tuple[int, int]) -> bool:
        """Determine if we should replan our goal"""
        # Emergency replan if in danger
        if self.is_scout and self.is_in_danger(current_pos):
            return True
        
        # Replan if no current goal
        if self.current_goal is None:
            return True
        
        # Replan if we reached the goal
        if current_pos == self.current_goal:
            return True
        
        # Replan periodically
        if self.step - self.last_goal_planning_step >= self.goal_replanning_interval:
            return True
        
        # Replan if path is blocked or empty
    ...
    def rl(self, observation_json: dict) -> int:
        """Main RL decision function using MCTS for planning + A* for navigation"""
        ...
        # Check for emergency escape mode for scouts
        if self.is_scout and self.is_in_danger(current_pos):
            self.emergency_mode = True
        elif not self.curr_guard_pos:
            self.emergency_mode = False

        # Strategic goal planning (MCTS)
        if self.should_replan_goal(current_pos):
            if self.is_scout and self.emergency_mode:
                # Emergency escape: find safe position
                self.current_goal = self.astar_navigator.find_escape_position(
                    current_pos, self.curr_guard_pos, self.obs_wall
                )
            else:
                # Normal strategic planning
                self.current_goal = self.mcts_planner.mcts_search(
                    current_pos, self.value, self.curr_guard_pos, self.is_scout,
                    iterations=60 if self.emergency_mode else 40
                )
            
            self.last_goal_planning_step = self.step
            
            # Plan path to new goal (A* with guard avoidance)
            self.goal_path = self.astar_navigator.astar_path_with_avoidance(
                current_pos, current_dir, self.current_goal, self.obs_wall,
                self.curr_guard_pos, self.guards_tracker, self.is_scout
            )

        # Tactical navigation (A* only)
        if self.goal_path:
            action = self.goal_path.pop(0)
            return action
        else:
            # No path available - try immediate escape or exploration
            if self.is_scout and self.curr_guard_pos:
                # Try to move away from nearest guard
                escape_goal = self.astar_navigator.find_escape_position(
                    current_pos, self.curr_guard_pos, self.obs_wall, max_distance=3
                )
                escape_path = self.astar_navigator.astar_path_with_avoidance(
                    current_pos, current_dir, escape_goal, self.obs_wall,
                    self.curr_guard_pos, self.guards_tracker, self.is_scout
                )
                if escape_path:
                    return escape_path[0]
            
            # Default action: move forward if possible, otherwise turn
            if not self.obs_wall[current_dir][current_pos[0]][current_pos[1]]:
                # Check if forward movement is safe for scouts
                ...
                if (self.astar_navigator.is_valid_position(forward_pos) and
                    (not self.is_scout or not self.curr_guard_pos or 
                     min(abs(forward_pos[0] - gx) + abs(forward_pos[1] - gy) 
                         for gx, gy in self.curr_guard_pos) > 1)):
                    return 0  # Move forward
            
            # Can't move forward safely, turn right
            return 3